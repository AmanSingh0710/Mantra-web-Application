...
const generateInvoice = require("../../utils/invoice/generateInvoice");
  ...
  try {
    ...
    const [orders, total] = await Promise.all([
      Order.find(query)
        ...
        .populate("products.storeId", "shopName")
        .populate("deliveryManId", "name mobile")
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit),
  ...
  try {
    ...
    const [orders, total] = await Promise.all([
      Order.find(query)
        ...
        .skip(skip)
        .limit(limit)
        .populate("products.productId", "productName thumbnail slug")
        .select(
          "orderNumber status products pricing payment createdAt tracking"
        ),
...
exports.assignDeliveryBoy = async (req, res) => {
  try {
    ...
    if (order.status !== "Packed") {
      return res.status(400).json({
        success: false,
        message: "Only packed orders can be assigned"
      });
    }
...
exports.getOrderDetails = async (req, res) => {
  try {
    ...
    const order = await Order.findById(orderId)
      .populate("products.productId", "productName slug thumbnail")
      .populate("deliveryManId", "name mobile")
      .populate("products.storeId", "storeName")
      .populate("userId", "name email mobile");
