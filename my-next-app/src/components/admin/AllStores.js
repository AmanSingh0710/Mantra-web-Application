...
export default function StorePage() {
  ...
  const [previews, setPreviews] = useState({ vendorImage: null, shopLogo: null, shopBanner: null, });
  ...
  const handleUpdate = async (id) => {
    ...
    Object.keys(form).forEach((key) => {
      if (key !== "password" && key !== "confirmPassword" && form[key]) {
        formData.append(key, form[key]);
      }
    });
              ...
              {stores.length > 0 ? (
                stores.map((store, index) => (
                  <tr key={store._id} className="hover:bg-gray-50 text-sm transition-colors">
                      ...
                      <div className="flex justify-center gap-3 text-gray-400">
                        ...
                        <Eye
                          ...
                          onClick={() => {
                            ...
                            setPreviews({
                              vendorImage: store.vendorImage?.url || null,
                              shopLogo: store.shopLogo?.url || null,
                              shopBanner: store.shopBanner?.url || null,
                            });
                    ...
                    <td className="px-6 py-4 text-center">
                      <img
                        src={store.vendorImage?.url || "/no-image.png"}
                        alt="Vendor"
          ...
          <div className="bg-[#f7f8fa] w-full max-w-6xl rounded-lg shadow-xl my-8">
            <div className="p-4 border-b bg-white rounded-t-lg flex justify-between items-center">
              <h2 className="text-base font-bold flex items-center gap-2">
                <User size={18} />{isEditing ? "Update Vendor" : "Add New Vendor"}
              </h2>
              <button
                onClick={() => {
                  resetForm();
            ...
            <form onSubmit={isEditing ? (e) => { e.preventDefault(); handleUpdate(editingId); } : handleSubmit} className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <div className="space-y-4">
                    <div>
                      ...
                      <input name="firstName" value={form.firstName} onChange={handleChange} className="w-full border border-[#ced4da] rounded p-2 text-sm outline-none focus:border-[#007bff]" placeholder="Ex: Jhone" />
                    </div>
                    <div>
                      <label className="text-[13px] block mb-1.5">Last Name</label>
                      <input name="lastName" value={form.lastName} onChange={handleChange} className="w-full border border-[#ced4da] rounded p-2 text-sm outline-none focus:border-[#007bff]" placeholder="Ex: Doe" />
                    </div>
                    <div>
                      ...
                      <div className="flex border border-[#ced4da] rounded overflow-hidden">
                        ...
                        <input name="mobile" value={form.mobile} onChange={handleChange} className="w-full p-2 text-sm outline-none" placeholder="Enter phone number" />
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-col items-center justify-center">
                    <div className="w-40 h-40 border-2 border-dashed border-[#dee2e6] rounded-lg mb-4 flex items-center justify-center bg-[#f8f9fb] overflow-hidden">
                      {previews.vendorImage ? <img src={previews.vendorImage} className="w-full h-full object-cover" /> : <Upload className="text-slate-300" size={40} />}
                    </div>
              ...
              <div className="bg-white p-6 rounded-lg border border-[#eff2f7] shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-6">
                    <div>
                      ...
                      <input name="shopName" value={form.shopName} onChange={handleChange} className="w-full border border-[#ced4da] rounded p-2 text-sm outline-none focus:border-[#007bff]" placeholder="Ex: Jhone" />
                    </div>
                    <div className="flex flex-col items-center">
                      ...
                      <div className="w-full">
                        ...
                        <div className="flex border rounded overflow-hidden text-sm">
                          <div className="flex-grow px-3 py-2 text-slate-400 truncate bg-white"> {form.shopLogo?.name || "Upload Logo"}</div>
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <textarea name="shopAddress" value={form.shopAddress} onChange={handleChange} className="w-full border border-[#ced4da] rounded p-2 text-sm outline-none focus:border-[#007bff] h-[38px] resize-none" placeholder="Ex: Doe"></textarea>
                    </div>
                    <div className="flex flex-col items-center">
                      ...
                      <div className="w-full">
                        ...
                        <div className="flex border rounded overflow-hidden text-sm">
                          <div className="flex-grow px-3 py-2 text-slate-400 truncate bg-white"> {form.shopBanner?.name || "Upload Banner"}</div>
                          <label className="bg-[#f8f9fb] px-4 py-2 border-l cursor-pointer">Browse
                ...
                <div className="flex justify-end gap-3 mt-8">
                  <button type="reset" onClick={resetForm} className="px-6 py-2 bg-[#f1f3f5] text-[#495057] rounded text-sm font-medium hover:bg-[#e9ecef]">Reset</button>
                  <button type="submit" className="px-6 py-2 bg-[#007bff] text-white rounded text-sm font-medium hover:bg-[#0069d9]">{isEditing ? "Update Vendor" : "Add Vendor"}</button>
                </div>
              </div>
            </form>
          </div>
