...
export default function AllOrders({ filter }) {
  ...
  const fetchDeliveryMen = async () => {
    try {
      const res = await fetchFromAPI("/deliveryman/list", {
        method: "GET",
      });
  ...
  const handleAssign = async (orderId, deliveryManId) => {
    try {
      const res = await fetchFromAPI("/order/assign-delivery-boy",
        {
          method: "PATCH",
                ...
                <tr key={order._id} className="hover:bg-gray-50/80 transition-colors">
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-2 text-xs text-gray-700 font-semibold">
                      ...
                      {[...new Set(
                        order.products ?.map((p) => p.storeId?.shopName)
                          .filter(Boolean)
                      )].join(", ") || "N/A"}
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-4">
                    ...
                    <div className="text-[10px] text-gray-500">
                      MRP : ₹{order.pricing?.subtotal?.toFixed(2) || "0.00"}
                    </div>

                    <span
                      className={`text-[9px] px-1.5 py-0.5 rounded font-bold uppercase ${order.payment?.status === "Paid"
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto relative">
            ...
            <div className="flex justify-between items-center p-6 border-b sticky top-0 bg-white z-10">
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={() => handleInvoiceDownload(selectedOrder._id)}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between text-gray-900 border-b pb-2">
                    ...
                    <span>₹{selectedOrder.pricing?.deliveryCharge?.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-lg font-black text-gray-900 pt-2">
                    <span>Total</span>
                    <span className="text-blue-600">₹{selectedOrder.pricing?.grandTotal?.toFixed(2)}</span>
                  </div>
                </div>
