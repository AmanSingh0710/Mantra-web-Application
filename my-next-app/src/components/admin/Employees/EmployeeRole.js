    ...
    <div className="p-6 bg-[#f4f7f9] min-h-screen text-gray-800">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {employees.map((emp) => {
          ...
          const detectedRole = emp.role || (emp.deliveryman_image ? "DELIVERY" : "UNKNOWN");
          const config = ROLE_CONFIG[detectedRole] || ROLE_CONFIG.UNKNOWN;

          // 2. Handle Image Logic: Check both possible image keys
          const profileImage = emp.employeeImage || emp.deliveryman_image;
          const imagePath = profileImage ? `${BASE_URL}/${profileImage}` : null;
          ...
          return (
            <div key={emp._id} className="group relative bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all">
              ...
              <div className={`w-12 h-12 ${config.color} rounded-lg flex items-center justify-center mb-4 transition-transform group-hover:scale-110`}>
                ...
                {imagePath ? (
                  <img src={imagePath} className="w-full h-full object-cover rounded-lg" alt="profile" />
                ) : (
                  config.icon
                )}
              </div>
              ...
              {emp.totalOrders !== undefined && (
                <div className="mb-4 text-xs font-semibold text-blue-600">
                  Total Deliveries: {emp.totalOrders}
                </div>
              )}

              {/* Status & Phone */}
              <div className="pt-4 border-t border-gray-100 flex items-center justify-between">
                <span className={`text-xs px-2 py-1 rounded-full ${emp.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                  {emp.status}
                </span>
                <span className="text-xs text-gray-400">{emp.phoneNumber || emp.phone}</span>
              </div>
