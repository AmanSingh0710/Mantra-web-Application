...
export default function DashboardHome({ setActiveTab }) {
  ...
  const [orderFilter, setOrderFilter] = useState("This Year");
  const [earningFilter, setEarningFilter] = useState("This Year");
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const period = periodMap[orderFilter];
  ...
  return (
    ...
    <div className="p-4 md:p-6 lg:p-8 bg-[#f8f9fb] min-h-screen animate-in fade-in duration-500">
      ...
      {refreshing && (
        <div className="fixed top-5 right-5 z-50 rounded-lg bg-white px-4 py-2 shadow-lg border text-sm font-medium">
          Updating Dashboard...
        </div>
      )}
              ...
              <div className="flex bg-gray-100 p-1 rounded-xl w-full sm:w-auto overflow-x-auto no-scrollbar">
                {['This Year', 'This Month', 'This Week'].map((tab) => (
                  <button
                    ...
                    className={`flex-1 sm:flex-none px-3 md:px-4 py-1.5 text-[10px] md:text-xs font-bold rounded-lg transition-all whitespace-nowrap ${orderFilter === tab
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full xl:w-auto">
            ...
            <div className="flex bg-gray-100 p-1 rounded-xl w-full sm:w-auto overflow-x-auto no-scrollbar">
              {['This Year', 'This Month', 'This Week'].map((tab) => (
                <button
                  key={tab}
                  onClick={() => setEarningFilter(tab)}
                  className={`flex-1 sm:flex-none px-4 py-1.5 text-[10px] md:text-xs font-bold rounded-lg transition-all ${earningFilter === tab ? 'bg-white text-[#007bff] shadow-sm' : 'text-gray-500 hover:text-gray-700'
                    }`}
                >
                  {tab}
                </button>
