                ...
                <div className="bg-gray-50/70 px-4 py-3 border-b border-gray-100 flex flex-wrap items-center justify-between gap-2 text-xs text-gray-500">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="hidden sm:inline">
                      Items: {order.items?.length || 0} {(order.items?.length || 1) === 1 ? 'package' : 'packages'}
                    </span>
                  </div>
                  ...
                  <div className="space-y-2 flex-grow">
                    <div className="flex items-baseline space-x-2">
                      ...
                      {order.payment?.method && (
                        <span className={`text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded ${order.payment?.method.toLowerCase() === 'cod' ? 'bg-amber-100 text-amber-800' : 'bg-green-100 text-green-800'
                          }`}>
                          {order.payment?.method}
                        </span>
                      )}
                    ...
                    {order.shipping?.address && (
                      <div className="text-xs sm:text-sm text-gray-600 max-w-xl">
                        <p className="font-semibold text-gray-900">{order.userId?.name || "Customer Drop"}</p>
                        <p className="text-gray-900 truncate mt-0.5">
                          {order.shipping?.address}
